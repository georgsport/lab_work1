...
*/
void exerciseTwo() {
	int* h = new int; //высота треугольника в строчках
	...
	std::cout << "Введите нужное количество строк треугольника: ";
	std::cin >> *h;
	std::cout << '\n';
	
	//основной цикл вывода треугольника
	for (*i = 0; *i <= *h; ++*i) {
...
// n - номер ряда сверху вниз
// i - номер числа в ряду
// Возвращает соответствующее число из треугольника Хосойа (Фибоначчи)
int hosoyaNumber(int* n, int* i) {
	//формула с википедии
	return fibNumber(*i + 1) * fibNumber(*n - *i + 1);
}

//Метод для получения n-го числа Фибоначчи рекурсивным способом.
int fibNumber(int n) {
	if (n == 0) return 0;
	if (n == 1) return 1;
